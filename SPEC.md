...
- publishedはMarkdown先頭のYAML Front Matterから取得する
...
Front Matterの`published`がBoolの場合、その値を記事の公開状態として保持する。
...
`published`キーがない場合はエラーにせず、`false`として扱わない。
...
- slug: 通常どおり表示
- Status: `⚠ Front Matter Error`

記事を選択した場合は、Workspace View上部に解析エラーを表示する。

### 8.8 Markdownを読み取れない場合
...
Markdownファイルの更新日時を内部データとして保持し、記事一覧の`Date Modified`列へ表示する。

日時はmacOSのロケールに従い、年、月、日、時、分を表示する。

更新日時を取得できない場合は`—`を表示する。

### 8.10 並び順

初期状態ではDate Modifiedの降順とし、更新日時が新しい記事を上に表示する。

Title、Slug、Status、Date Modifiedの列見出しから、各列の昇順または降順へ変更できるようにする。

titleとslugの比較は大文字小文字を区別せず、ローカライズされた比較を使用する。

titleが同じ場合はslugの昇順とする。

Statusの昇順は`Draft`、`Published`、`—`、エラーの順とする。降順では`Published`、`Draft`の順を反転するが、`—`とエラーは末尾に維持する。

更新日時を取得できない記事は、昇順と降順のどちらでも末尾に表示する。

選択した列の値が同じ場合はslugの昇順とする。
...
| Status | `Published`、`Draft`、`—`、またはエラー状態 |
| Date Modified | Markdownファイルの更新日時 |

Status列では、エラー状態を警告アイコンとテキストの両方で示す。

### 9.2 Statusフィルタ

画面上部にStatusフィルタを設置する。
...
|---|---|
| All | すべての記事 |
| Draft | `published: false`の記事 |
| Published | `published: true`の記事 |
| Unset | `published`キーがなく、読み取りエラーまたはFront Matterエラーではない記事 |
| Errors | 読み取りエラーまたはFront Matterエラーの記事 |

初期状態は`All`とする。

Statusフィルタと検索条件は同時に適用する。
...
実行時は、選択中のリポジトリルートの絶対パスだけをクリップボードへ書き込む。
`cd`、引用符、改行は付加しない。
...
```text
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│ SlugDock                            [Repository] [Copy Path] [Reload]                    │
├─────────────────────────────────────────────────────────────────────────────────────────┤
│ [ Search by title or slug                              ] [Status: All]                  │
├──────────────────────┬───────────────────┬─────────────┬────────────────────────────────┤
│ Title                │ Slug              │ Status      │ Date Modified                  │
├──────────────────────┼───────────────────┼─────────────┼────────────────────────────────┤
│ Zenn CLIの使い方     │ example-article   │ Published   │ 2026/07/14 09:30               │
│ SwiftUIでアプリを作る│ swiftui-app       │ Draft       │ 2026/07/13 18:15               │
└──────────────────────┴───────────────────┴─────────────┴────────────────────────────────┘
```
...
- キーボードだけで主要操作を実行できる
- 選択状態を色だけで表現しない
- VoiceOverでtitle、slug、Status、画像ファイル名を読み取れる
...
- `published: true`と`published: false`をBoolとして取得できる
- `published`キーがない場合に未設定として扱い、エラー状態にならない
- `published`がBool以外の場合にエラー状態を返す
...
- titleまたはslugによる検索とStatusフィルタを同時に適用できる
- `All`、`Draft`、`Published`、`Unset`、`Errors`で正しく絞り込める
- エラー記事を`Unset`として扱わない
- Title、Slug、Status、Date Modifiedを昇順と降順で並べ替えられる
- 未設定の更新日時、`Unset`、エラー記事が規定どおり末尾になる
- 同値の場合にslugの昇順で安定した順序になる
...
- 無効なリポジトリを登録しない
- リポジトリルートの絶対パスだけをクリップボードへコピーできる

### 26.2 記事一覧

- `articles/*.md`のtitle、slug、Status、変更日が表示される
- titleまたはslugで検索できる
- `All`、`Draft`、`Published`、`Unset`、`Errors`で記事を絞り込める
- Title、Slug、Status、Date Modifiedの各列で並べ替えられる

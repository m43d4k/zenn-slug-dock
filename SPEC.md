...
```text
選択したフォルダに articles ディレクトリがありません。
```
...
以下の場合は表示タイトルを`タイトル未設定`とする。

- `title`キーがない
- `title`が空文字
- `title`が空白だけ

Front Matter自体がない場合も、表示タイトルを`タイトル未設定`とし、エラー状態にはしない。
...
記事を選択した場合は、執筆モード上部に読み取りエラーを表示する。
...
`画像名を変更…`または`Return`を実行すると、現在の拡張子を除いたファイル名を初期値とする入力シートを表示する。
...
執筆モード中のMarkdownファイルが存在しなくなった場合は、記事一覧へ戻り、次を表示する。
...
│        example-article                               │
├──────────────────────────────────────────────────────┤
│ [MDパスをコピー] [MDをFinderで表示]                  │
│ [画像フォルダパスをコピー] [画像フォルダを開く]      │
├──────────────────────────────────────────────────────┤
│                                                      │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐     │
...
- titleが空の場合に`タイトル未設定`となる
- Front Matterがない場合に`タイトル未設定`となり、エラー状態にならない
